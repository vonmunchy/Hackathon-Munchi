...
import (
	...
	txcmd "github.com/BML-Digital/swipe-merchants-dev/cli/internal/cli/transactions"
...
func run(t *testing.T, home, port string, args ...string) (string, error) {
	...
	root := cli.NewRoot(g, func(r *cobra.Command, gf *cli.GlobalFlags) {
		r.AddCommand(txcmd.NewCommand(gf))
	})
	root.SetArgs(append([]string{"--port", port, "transactions"}, args...))
	root.SetOut(stdout)
	root.SetErr(stderr)
...
func TestTransactions_History_EmptyResult(t *testing.T) {
	port, st := liveMock(t)
	home := t.TempDir()
	seedTokenCache(t, home, "http://localhost:"+port, st, []string{"transactions:history"})

	stdout, err := run(t, home, port, "history")
	if err != nil {
...
func TestTransactions_History_WrongScope_Errors(t *testing.T) {
	port, st := liveMock(t)
	home := t.TempDir()
	seedTokenCache(t, home, "http://localhost:"+port, st, []string{"wallet:balance"})
	_, err := run(t, home, port, "history")
